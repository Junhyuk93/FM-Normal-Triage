...
from dinov2.data.transforms import srpark_make_classification_eval_transform, srpark_make_classification_train_transform
import dinov2.distributed as distributed
from dinov2.eval.metrics import MetricType, build_metric, build_reg_metric
...
def get_args_parser(
    ...
    parser.set_defaults(
        ...
        val_metric_type= None, #MetricType.MEAN_ACCURACY, #,MEAN_PER_CLASS_ACCURACY
...
class LinearPostprocessor(nn.Module):
    def __init__(self, linear_regressor, class_mapping=None):
        super().__init__()
...
def setup_linear_regressors(sample_output, n_last_blocks_list, learning_rates, batch_size, num_classes=1):
    linear_regressors_dict = nn.ModuleDict()
    optim_param_groups = []
    ...
    linear_regressors = AllRegressors(linear_regressors_dict)
...
def evaluate_linear_regressors(
    ...
    metric = build_reg_metric()
    postprocessors = {k: LinearPostprocessor(v, class_mapping) for k, v in linear_regressors.regressors_dict.items()}
    ...
    for i, (regressor_string, metric) in enumerate(results_dict_temp.items()):
        ...
        if (
            best_regressor_on_val is None and metric.item() > max_r2
        ) or regressor_string == best_regressor_on_val:
            max_r2 = metric.item()
            best_regressor = regressor_string

    results_dict["best_regressor"] = {"name": best_regressor, "R2": max_r2}
...
def eval_linear(
    *,
    ...
    for data, labels in metric_logger.log_every(
        ...
        labels = labels.unsqueeze(dim=1).cuda(non_blocking=True)

        features = feature_model(data)
        outputs = linear_regressors(features)

        losses = {f"loss_{k}": nn.L1Loss()(v, labels) for k, v in outputs.items()}  ### Regression에 맞게 loss 수정: L1 loss
...
def make_eval_data_loader(test_dataset_str, batch_size, num_workers, metric_type):
    test_dataset = make_dataset(
        ...
        transform=srpark_make_classification_eval_transform(),
    )
...
def test_on_datasets(
    ...
    for test_dataset_str, class_mapping, metric_type in zip(test_dataset_strs, test_class_mappings, test_metric_types):
        ...
        dataset_results_dict = evaluate_linear_regressors(
            feature_model,
        ...
        results_dict[f"{test_dataset_str}_R2"] = 100.0 * dataset_results_dict["best_regressor"]["R2"]
    return results_dict
...
def run_eval_linear(
    ...
    val_class_mapping_fpath=None,
    test_class_mapping_fpaths=[None],
    val_metric_type= None, #MEAN_PER_CLASS_ACCURACY,
    test_metric_types=None,
):
    seed = 0
    ...
    training_num_classes = 1 ## regression!
    ...
    linear_regressors, optim_param_groups = setup_linear_regressors(
        sample_output,
        n_last_blocks_list,
        learning_rates,
        batch_size,
    ...
    results_dict[f"{val_dataset_str}_R2"] = 100.0 * val_results_dict["best_regressor"]["R2"]
