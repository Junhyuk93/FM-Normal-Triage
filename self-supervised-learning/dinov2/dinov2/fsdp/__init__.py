...
def get_fsdp_wrapper(model_cfg, modules_to_wrap=set()):
    ...
    """
    log.out에서 UserWarning: FSDP is switching to use `NO_SHARD` 
    instead of ShardingStrategy.SHARD_GRAD_OP since the world size is 1.
    라는 로그가 있어서 위 코드를 아래 코드로 바꿔서 실행
    결과:
