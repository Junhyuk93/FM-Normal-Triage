...
class DINOv2_MultiModelExtractor(nn.Module):
    ...
    def __init__(self, args, n_blocks=4, avg_pool=True, linear_ckpts=None):
        ...
        print(f"Loaded {self.num_models} individual models for 3-class probability extraction")

    @torch.no_grad()
    def extract_svm_features(self, x):
        """
        각 모델에서 3-class 확률을 모두 추출하여 연결 (13 models × 3 classes = 39 features)
        """
        # backbone에서 intermediate features 추출
        x_tokens_list = self.feature_model(x)

        # 각 모델에서 3-class 확률 추출
        all_probs = []
...
def main():
    ...
    print(f"Individual models: 13 × 3-class probabilities used as SVM input")
    return results
