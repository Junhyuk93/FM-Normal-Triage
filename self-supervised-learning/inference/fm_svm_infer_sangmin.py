...
#            labels_bin.extend([to_binary(int(v)) for v in label_np])
    with torch.no_grad():
        for batch in tqdm(loader, desc="Infer"):
            ...
            # --- 여기부터 수정 ---
            # 이 데이터셋은 모두 normal(0)이라고 가정한다.
            # 실제 label 값은 무시하고, 배치 크기만큼 0을 채운다.
...
# metrics (binary only)
    if labels_bin.size > 0:
        ...
        # 방법 1: samples 속성에서 파일 경로 추출 (ImageFolder 등)
        ...
        # 방법 3: 데이터셋 클래스에서 직접 파일 리스트 속성 찾기
        ...
        # 방법 4: 데이터셋 root에서 직접 DICOM 파일 스캔
            ...
            all_paths = sorted(all_paths)  # 정렬하여 순서 보장
            print(f"[Info] Found {len(all_paths)} DICOM files by scanning")

        # 방법 5: 마지막 시도 - 데이터셋 디버깅
        else:
