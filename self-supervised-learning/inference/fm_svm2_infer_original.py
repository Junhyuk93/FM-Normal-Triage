...
# ------------------ FM Feature Extractor ------------------
class DINOv2_MultiModelExtractor(nn.Module):
    """
    13개의 개별 DINOv2 모델에서 각각 3-class logits을 추출하여 SVM 입력으로 사용
    """
    def __init__(self, args, n_blocks=4, avg_pool=True, linear_ckpts=None):
        ...
        # 13개의 개별 linear classifiers (각각 3-class 출력)
        ...
        print(f"Loaded {self.num_models} individual models for SVM feature extraction")

    @torch.no_grad()
    def extract_svm_features(self, x):
        """
        각 모델에서 3-class logits을 추출하여 연결 (13 models × 3 classes = 39 features)
        """
        # backbone에서 intermediate features 추출
        x_tokens_list = self.feature_model(x)
        
        # 각 모델에서 3-class logits 추출
        all_logits = []
        ...
        # 모든 logits를 연결: (batch_size, 13*3) = (batch_size, 39)
        if len(all_logits) > 0:
            combined_logits = torch.cat(all_logits, dim=-1)  # (batch_size, 39)
        else:
            raise ValueError("No classifiers loaded!")
        
        print(f"Debug: Combined logits shape: {combined_logits.shape}")  # 디버그용
        return combined_logits.cpu().numpy()
...
def run_fm_svm_inference(args):
    ...
    # FM Feature Extractor 초기화 (13개 모델)
    ...
    if len(linear_list) != 13:
        ...
        print("Make sure you have exactly 13 linear checkpoint files")
    ...
    # Feature 추출 및 예측
    print("Extracting 13×3 logits features for SVM...")
    all_svm_features = []
    all_labels = []
    ...
    with torch.no_grad():
        for image, label in tqdm(dataloader):
            ...
            # 13개 모델에서 각각 3-class logits 추출 → SVM 입력용 39차원 feature
    ...
    if len(all_labels) > 0:  # 라벨이 있는 경우
        ...
        for model_idx in range(len(linear_list)):
            ...
            print(f"Model {model_idx+1:2d}: {model_acc * 100:.2f}%")
...
def main():
    ...
    print(f"Individual models: 13 × 3-class predictions used as SVM input")
    return results
