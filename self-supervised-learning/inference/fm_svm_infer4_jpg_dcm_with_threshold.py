...
def setup_multi_format_loader(data_path, batch_size=1, num_workers=8, img_size=(1024, 1024)):
    ...
    # image_paths를 samples로 저장 (파일 경로 추출용)
    dataset.samples = [(path, label) for path, label in zip(image_paths, labels)]
...
def run(args):
    ...
    # ========== THRESHOLD 적용 코드 시작 ==========
    threshold = getattr(args, 'svm_threshold', 0.5)
    print(f"\n[Threshold] Custom threshold: {threshold}")

    # SVM 확률/decision score 추출
    probs_abnormal = None
    ...
    if hasattr(svm, 'decision_function'):
        ...
        # Binary classification: decision_scores는 1D array
        if len(decision_scores.shape) == 1:
            probs_abnormal = expit(decision_scores)  # sigmoid 적용
            preds = (probs_abnormal >= threshold).astype(int)
            
            print(f"[Threshold] Decision scores range: [{decision_scores.min():.3f}, {decision_scores.max():.3f}]")
            print(f"[Threshold] Probs (abnormal) range: [{probs_abnormal.min():.3f}, {probs_abnormal.max():.3f}]")
        else:
            # Multi-class
            probs = softmax(decision_scores, axis=1)
            probs_abnormal = probs[:, 1]
            preds = (probs_abnormal >= threshold).astype(int)

    elif hasattr(svm, 'predict_proba'):
        probs = svm.predict_proba(features)
        probs_abnormal = probs[:, 1]  # abnormal class
        preds = (probs_abnormal >= threshold).astype(int)
        
        print(f"[Threshold] Probs (abnormal) range: [{probs_abnormal.min():.3f}, {probs_abnormal.max():.3f}]")
    else:
        print("[Warning] SVM doesn't support probability. Using default predict().")
        preds = svm.predict(features).astype(int)
        threshold = None

    # Threshold 효과 출력
    if threshold is not None and probs_abnormal is not None:
        default_preds = (probs_abnormal >= 0.5).astype(int)
        print(f"\n[Threshold Impact]")
        print(f"  Default (0.5):  {default_preds.sum():4d} abnormal predictions ({default_preds.sum()/len(default_preds)*100:.1f}%)")
        print(f"  Custom ({threshold:.2f}): {preds.sum():4d} abnormal predictions ({preds.sum()/len(preds)*100:.1f}%)")
        diff = preds.sum() - default_preds.sum()
        print(f"  Difference:     {diff:+4d} predictions changed")
    # ========== THRESHOLD 적용 코드 끝 ==========

    # SVM 확률 재계산 (CSV 저장용)
    if probs_abnormal is not None:
        prob_class_0 = 1 - probs_abnormal
        prob_class_1 = probs_abnormal
        svm_probabilities = np.stack([prob_class_0, prob_class_1], axis=1)
    elif hasattr(svm, 'decision_function') and decision_scores is not None:
        if len(decision_scores.shape) == 1:
    ...
    try:
        ...
        for i in range(len(preds)):
            row = {
                ...
                'threshold_used': threshold if threshold is not None else 0.5,
            }
            
            if decision_scores is not None and len(decision_scores.shape) == 1:
...
def build_argparser():
    ...
    # ✅ Threshold 인자 추가
    ap.add_argument("--svm-threshold", type=float, default=0.5,
                    help="SVM decision threshold for binary classification (default: 0.5). "
...
def main():
    ...
    _ = run(args)
    print(f"\n[Inference completed] Multi-format images → 13×3 probs → SVM (threshold={getattr(args, 'svm_threshold', 0.5)}) → binary metrics printed.")
